...
exports.insertUser = function (user, func) {
    ...
    exports.checkExistingUser(user, function (result) {
        ...
        var sql = "INSERT INTO user (first_name, last_name,username, password, email, client_id) VALUES (?,?,?,?,?,?);";
        ...
        con.query(sql, [user.firstName, user.lastName, user.username, user.password, user.email, user.clientId], function (err, result) {
            if (err) throw err;
...
exports.getClientCollection = function (res, func) {
    ...
    con.query(sql, function (err, result) {
        ...
        for (var i = 0; i < result.length; i++) {
            clientCollection.push(result[i]);
        }
        console.log(result);
        func(res, err, clientCollection);
    });

};

exports.getBulletinCollection = function (res, clientId, settings, func) {

    var bulletinCollection = [];
    var sql = '';
    ...
    if (perPage != 0) {
        sql = "SELECT * FROM bulletin WHERE client_id = ? AND active = 1 LIMIT " + offset + " , " + perPage + " ";
    } else {
        sql = "SELECT * FROM bulletin WHERE client_id = ? AND active = 1";
    }

    con.query(sql, [parseInt(clientId)], function (err, result) {

        if (err) throw err;

        var sqlAll = " SELECT count(*) as count FROM bulletin WHERE client_id = ? AND active = 1";
        con.query(sqlAll, [parseInt(clientId)], function (error, resp) {
            if (error) throw error;
            response.count = resp[0].count;

            for (var i = 0; i < result.length; i++) {
                bulletinCollection.push(result[i]);
            }
            response.data = bulletinCollection;
            console.log(response)
            func(res, err, response);
        });
    });
};

exports.getLinkCollection = function (res, obj, func) {

    var params = [];
    params.push(parseInt(obj.clientId));
    var offset = parseInt(obj.offset);
    var perPage = parseInt(obj.perPage);
    var bulletinId = parseInt(obj.bulletinId);
    ...
    if (bulletinId != 0) {
        var sqlAll = "SELECT count(*) as count FROM market_bulletins.link JOIN market_bulletins.bulletin ON link.bulletin_id = bulletin.id WHERE bulletin.client_id = ? AND bulletin.id = ?";
        params.push(parseInt(bulletinId));
    }
    else {
        var sqlAll = "SELECT count(*) as count FROM market_bulletins.link JOIN market_bulletins.bulletin ON link.bulletin_id = bulletin.id WHERE bulletin.client_id = ?";
    }

    con.query(sqlAll, params, function (error, resp) {

        if (error) throw error;
        count = resp[0].count;

        var sql = "SELECT DISTINCT link.id as linkId, link.target_link,  link.track_link,  link.description,  link_bulletin.bulletin_id, link.visits, client.name, client.id as clientId, bulletin.title FROM market_bulletins.link " +
            " JOIN market_bulletins.link_bulletin " +
            " ON link_bulletin.link_id = link.id  " +
            " JOIN market_bulletins.bulletin " +
            " ON link_bulletin.bulletin_id = bulletin.id " +
            " JOIN market_bulletins.client " +
            " ON client.id = bulletin.client_id " +
            " HAVING client.id = ? "

        if (parseInt(bulletinId) != 0) {
            // sql += " AND link.bulletin_id = ?  ORDER BY linkId ";
            sql += " AND link_bulletin.bulletin_id = ?  ORDER BY link.id  " + limit;
            //params.push(parseInt(bulletinId));
        } else {
            sql += " ORDER BY linkId  " + limit;
        }
...
exports.updateLink = function (id, res, func) {

    var sqlSelect = " SELECT * FROM market_bulletins.link WHERE link.id =? LIMIT 1; "
    con.query(sqlSelect, [parseInt(id)], function (err, result) {

        if (err) throw err;
        if (result) {
            var sqlUpdate = " UPDATE market_bulletins.link SET link.visits = link.visits + 1 WHERE link.id =?;"
            con.query(sqlUpdate, [parseInt(id)], function (error) {
                if (error) throw error;
                var sqlInsertTime = "INSERT INTO market_bulletins.link_timestamps SET link_timestamps.link_id = ? ;";
                con.query(sqlInsertTime, [parseInt(id)], function (er) {
                    if (error) throw error;
...
exports.updateLinkData = function (obj, func) {

    console.log(obj)

    var sqlUpdate = " UPDATE market_bulletins.link SET link.target_link = ?, link.description = ?, link.bulletin_id = ?  WHERE link.id =?;"
    con.query(sqlUpdate, [obj.targetLink, obj.description, parseInt(obj.bulletinId), parseInt(obj.editId)], function (error, result) {

        if (error) throw error;

        var sqlSelect = "SELECT * FROM market_bulletins.link_bulletin WHERE link_bulletin.link_id = ? AND link_bulletin.bulletin_id = ? LIMIT 1 ";

        con.query(sqlSelect, [parseInt(obj.editId), parseInt(obj.bulletinId)], function (er, response) {

            if (response.length == 0) {

                var sqlInsert = "INSERT INTO market_bulletins.link_bulletin (link_bulletin.link_id, link_bulletin.bulletin_id)  VALUES (?,?)";

                con.query(sqlInsert, [parseInt(obj.editId), parseInt(obj.bulletinId)], function (err) {
            ...
            // else {
            //     var sqlUpdateLinkBulletin = "UPDATE  market_bulletins.link_bulletin  SET link_bulletin.bulletin_id = ? WHERE link_bulletin.link_id = ?";

            //     con.query(sqlUpdateLinkBulletin, [parseInt(obj.bulletinId), parseInt(obj.editId)], function (err) {
...
exports.updateBulletinData = function (obj, func) {

    console.log(obj)

    var sqlUpdate = " UPDATE market_bulletins.bulletin SET bulletin.title = ?, bulletin.description = ?, bulletin.start_date =?, bulletin.expiration_date = ?,  bulletin.send_date = ?, bulletin.type = ?, bulletin.send_copies = ? WHERE bulletin.id =?;"
    con.query(sqlUpdate, [obj.title, obj.description, obj.startDate, obj.endDate, obj.sendDate, obj.type, obj.sendCopies, parseInt(obj.bulletinId)], function (error, result) {
...
exports.deleteLinkData = function (obj, func) {

    console.log(obj)

    var sqlUpdate = " DELETE FROM market_bulletins.link  WHERE link.id =?;"
    con.query(sqlUpdate, [parseInt(obj.id)], function (error, result) {
...
exports.updateBulletin = function (id, res, func) {

    var sqlSelect = " SELECT * FROM market_bulletins.bulletin WHERE bulletin.id = ? LIMIT 1; "
    con.query(sqlSelect, [parseInt(id)], function (err, result) {

        if (err) throw err;
        if (result) {
            var sqlUpdate = " UPDATE market_bulletins.bulletin SET bulletin.visits = bulletin.visits + 1 WHERE bulletin.id =?;"

            con.query(sqlUpdate, [parseInt(id)], function (error) {

                if (error) throw error;

                var sqlInsertTime = "INSERT INTO market_bulletins.bulletin_timestamps SET bulletin_id = ? ;";
...
exports.insertBulletin = function (res, obj, func) {

    var sql = "INSERT INTO market_bulletins.bulletin (bulletin.title, bulletin.description, bulletin.start_date, bulletin.expiration_date, bulletin.send_date, bulletin.type, bulletin.visits, bulletin.client_id, bulletin.send_copies) VALUES (?,?,?,?,?,?,?,?,?)";

    con.query(sql, [obj.title, obj.description, obj.startDate, obj.endDate, obj.sendDate, obj.type, 0, parseInt(obj.clientId), parseInt(obj.sendCopies)], function (err, result) {
...
exports.deleteBulletin = function (obj, func) {

    console.log(obj)

    var sqlUpdate = " UPDATE market_bulletins.bulletin SET bulletin.active = 0  WHERE bulletin.id =?;"
    con.query(sqlUpdate, [parseInt(obj.id)], function (error, result) {
...
exports.insertLink = function (res, obj, func) {

    var sql = "INSERT INTO market_bulletins.link (link.target_link, link.track_link, link.description, link.bulletin_id, link.visits) VALUES (?,?,?,?,?)";

    con.query(sql, [obj.targetLink, obj.trackLink, obj.description, parseInt(obj.bulletinId), 0], function (err, result) {

        if (err) throw err;
        var linkId = result.insertId;
        var sqlInsert = "INSERT INTO market_bulletins.link_bulletin (link_bulletin.link_id, link_bulletin.bulletin_id)  VALUES (?,?)";

        con.query(sqlInsert, [linkId, parseInt(obj.bulletinId)], function (err) {
